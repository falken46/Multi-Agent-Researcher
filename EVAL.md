...
> Candidate Recall@20 衡量候选生成，Hit@5 与 MRR@5 衡量最终前五名。重排无法挽回未进入 Top-20 的 gold，因此三者必须同时报告。
...
| 组 | Candidate Recall@20 | Hit@5 | MRR@5 |
|---|---:|---:|---:|
| R1 向量 | 93.64% | 96.00% | 0.7238 |
| R2 BM25 | 85.65% | 94.00% | 0.7575 |
| R3 RRF | 92.27% | 95.00% | 0.7777 |
| R4 + ONNX rerank | 92.27% | 93.00% | 0.7377 |

这组结果支持两个有限结论：

1. R3 相比 R1 的 MRR@5 增加 0.0538，但 Candidate Recall@20 与 Hit@5 分别下降
   1.37、1.00 个百分点；RRF 改善了首个相关 passage 的平均位置，但没有全面优于向量通道。
2. R4 与 R3 候选完全相同，Candidate Recall@20 因而不变；当前 reranker 使 Hit@5 下降
   2.00 个百分点、MRR@5 下降 0.0400，不能把它表述成有效优化，应保留可切换开关并继续分析模型/数据匹配。
